...
void parser_rd_msg_values(msg_data_st *msg_data, String *StrP)
{
    ...
    Serial.print(*StrP);
    Serial.printf(" msg_data->tag_indx %d\n",msg_data->tag_indx);  
    indx1 = indx2+1;
...
bool lora_receive_msg(void)
{
      ...
      if (Serial1.available())
      {
            ...
            switch(lora.role)
            {
               case NODE_ROLE_UNDEFINED: Serial.print("Alien"); break;
               case NODE_ROLE_CLIENT:    Serial.print("Server"); break;
               case NODE_ROLE_SERVER:    Serial.print("Client"); break;   
            }
   ...
   switch(lora_th.state)
   {
      ...
      case 11:
         if (millis() > timeout_ms) 
         {
            switch(lora.role)
            {
               case NODE_ROLE_UNDEFINED: lora_th.state = 0; break;
               case NODE_ROLE_CLIENT: lora_th.state = 20; break;
               case NODE_ROLE_SERVER: lora_th.state = 500; break;  
            }        
         }
         break;
      // ***************** Client Code ***************************    
